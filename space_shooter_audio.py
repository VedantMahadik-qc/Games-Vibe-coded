...
# --- SOUND: lazy init after first user interaction ---
SOUND_ON = False
shoot_sfx = explosion_sfx = hit_sfx = gameover_sfx = None

def init_audio():
    global SOUND_ON, shoot_sfx, explosion_sfx, hit_sfx, gameover_sfx
    if SOUND_ON:
        return
    try:
        pygame.mixer.init()
...
async def main():
    ...
    boss_threshold = 500; new_best = False
    enemy_spawn_rate = 90; level_score = 0

    def reset_game():
        nonlocal player, score, level, enemy_timer, boss_active, new_best, level_score, enemy_spawn_rate
        ...
        score=0; level=1; enemy_timer=0; boss_active=False
        new_best=False; level_score=0; enemy_spawn_rate=90
        if SOUND_ON:
            try:
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                init_audio()  # <-- triggers on first keypress, safe for browser
